...
import { Message, Role, StudentProfile, AppView, SimulationState, SubjectModule, NotebookTool, CourseMaterial, UserRole, BuilderProject } from './config/types';
...
const App: React.FC = () => {
  ...
  // 1. Load basic app state (Where was I?)
  ...
  // 2. Load Messages SPECIFIC to the active subject (if any)
  ...
  // Critical Fix: Only save messages to the CURRENT active subject + track
  useEffect(() => {
  ...
  // --- EFFECT: AI RESPONSE ANALYSIS (SIMULATION DETECTOR) ---
  useEffect(() => {
    if (messages.length > 0) {
      ...
      if (lastMsg.role === Role.MODEL) {
        ...
        // NEW: Parse Score from AI Response
        ...
        if (scoreMatch) {
            ...
            setStudent(prev => {
                ...
                // Avoid infinite loops or unnecessary updates if score is already recorded
            ...
            // Trigger Achievement Popup based on score
            ...
            // Trigger minor achievement for answering correctly in chat
            if (!triggeredAchievements.has(lastMsg.id)) {
                setTriggeredAchievements(prev => new Set(prev).add(lastMsg.id));
                setCurrentAchievement({
                   id: generateUUID(),
  ...
  const handleSendMessage = async (text: string, isSystemTrigger = false) => {
    ...
    if (isGemmaMode) {
      ...
      // Simulate client-side WebGPU acceleration compilation and execution latency
      setTimeout(async () => {
    ...
    const userMsg: Message = {
      id: generateUUID(),  // ✅ CORREGIDO: crypto.randomUUID() para IDs únicos
      role: Role.USER,
      text: text,
      timestamp: Date.now(),  // ✅ CORREGIDO: number en lugar de Date
      track: userRole  // ✅ AGREGADO: Para Dual-Track isolation
    };
    ...
    // ✅ CORREGIDO: Agregar track como 5to parámetro para Dual-Track isolation
    const aiResponseText = await sendMessageToGemini(context, text, activeSubject, userRole, userRole);
  ...
  const handleSmartDownload = () => {
    ...
    // ✅ CORREGIDO: Agregar parámetro track para Dual-Track isolation en exportación
    const success = DownloadService.generateOfflinePackage(activeSubject, messages, userRole);
    if (success) {
        console.log("✅ Paquete 'Preparar para la Vereda' descargado correctamente. Puedes abrirlo sin internet.");
  ...
  const handleEnterSubject = async (subject: string) => {
    if (subject === "Simulacro General ICFES") {
      ...
      const entryText = SIMULATION_TRIGGER_MESSAGE;
      // ✅ CORREGIDO: Agregar track como 5to parámetro
      const response = await sendMessageToGemini([], entryText, "SIMULACRO ICFES", userRole, userRole);
  ...
  const handleStartModuleTool = async (module: SubjectModule, tool: NotebookTool) => {
    ...
    const materials = StorageService.getCourseMaterials();
  ...
  // ✅ CORREGIDO: LandingPage handlers con valores lowercase para UserRole
  if (currentView === 'LANDING') {
    return (
      <LandingPage 
        onStart={() => {
          setUserRole('student');  // ✅ CORREGIDO: lowercase
          setCurrentView('CAMPUS');
        }} 
        onTeacherAccess={() => {
          setUserRole('teacher');  // ✅ CORREGIDO: lowercase
          setCurrentView('TEACHER_PORTAL');
        }}
        onConstructorAccess={() => {
          setUserRole('builder');  // ✅ CORREGIDO: 'builder' en lugar de 'CONSTRUCTOR'
  ...
  if (currentView === 'TEACHER_PORTAL') {
    return (
      <TeacherPortal onReturn={() => setCurrentView('LANDING')} />
    );
  }
    ...
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 overflow-hidden">
      ...
      <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-3 flex items-center justify-between shadow-sm z-10 sticky top-0 transition-colors duration-300">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Gemma 4 Local WebGPU Engine Indicator */}
          <button
            onClick={() => {
              if (!forceGemmaLocal) {
                // Simulate model download caching weights
            ...
            `}
            title={forceGemmaLocal ? "Operando en Gemma 4 Local (Inferencia local activa)" : "Cambiar a Inferencia Offline con Gemma 4"}
          >
            <span className="text-[11px]">🤖</span>
            {gemmaModelDownloading 
              ? `Compilando Gemma 4 Local... ${gemmaProgress}%` 
              : forceGemmaLocal 
                ? "Gemma 4 Local Activo" 
                : "Activar Gemma 4 Local"
            }
          </button>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <AnimatePresence mode="wait">
          {currentView === 'CAMPUS' ? (
              ...
              {/* SIMULATION PROGRESS BAR (ONLY SHOWS IN SIMULATION MODE) */}
                        ...
                        <div className="bg-white dark:bg-slate-950 p-3 rounded-xl shadow-inner border border-slate-200 dark:border-slate-800">
                          <audio 
                            ...
                            onTimeUpdate={(e) => {
                              ...
                              // Check if there are interaction points defined for this DBA code
                              ...
                              if (points) {
                                ...
                                if (matchedPoint) {
                                  ...
                                  if (!hasTriggeredPoint[uniqueKey]) {
                                    ...
                                    // Pausar automáticamente el podcast para que el estudiante responda en chat (Ping-Pong Híbrido)
                                    e.currentTarget.pause();
                                    
                                    // Añadir la pregunta correspondiente al chat
