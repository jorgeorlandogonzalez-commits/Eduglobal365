...
const App: React.FC = () => {
  // ==========================================================================
  // 🆕 ESTADO DE INICIALIZACIÓN ASÍNCRONA (IndexedDB)
  // ==========================================================================
  ...
  // --- STATE: Student Profile (con ZPD, Región, Gamificación) ---
  ...
  // ==========================================================================
  // 🆕 EFFECT: INICIALIZACIÓN ASÍNCRONA (IndexedDB + Perfil)
  // ==========================================================================
  useEffect(() => {
    const initializeApp = async () => {
      try {
        // 1. Cargar estado de la app desde IndexedDB
        const appState = await StorageService.loadAppState();
        if (appState) {
          setCurrentView(appState.currentView);
          setActiveSubject(appState.activeSubject);
          setUserRole(appState.userRole);
          
          // Cargar mensajes del subject activo
        ...
        // 2. Cargar perfil del estudiante desde IndexedDB
        const profile = await StorageService.getStudentProfile();
        if (profile) {
          setStudent(prev => ({ ...prev, ...profile }));
        } else {
          // Guardar perfil por defecto si no existe
          await StorageService.saveStudentProfile(student);
        }
        ...
        setInitError("Error cargando datos locales. Intenta recargar la página.");
        setIsInitializing(false);
      }
    };
  ...
  // ==========================================================================
  // 🆕 EFFECT: PERSISTENCE (IndexedDB - async)
  // ==========================================================================
  useEffect(() => {
    if (!isInitializing) {
      StorageService.saveAppState(currentView, activeSubject, userRole);
    }
  }, [currentView, activeSubject, userRole, isInitializing]);

  useEffect(() => {
    if (!isInitializing && activeSubject && messages.length > 0) {
      StorageService.saveSubjectChat(activeSubject, messages, userRole);
    }
  }, [messages, activeSubject, userRole, isInitializing]);

  // 🆕 Guardar perfil del estudiante cuando cambie
  useEffect(() => {
    if (!isInitializing) {
      StorageService.saveStudentProfile(student);
    }
  }, [student, isInitializing]);
        ...
        if (scoreMatch) {
            ...
            setStudent(prev => {
                ...
                const newGamification: GamificationState = {
                  ...(prev.gamification || { streak: 0, lastActiveDate: '', badges: [], totalPoints: 0, level: 1 }),
                  totalPoints: (prev.gamification?.totalPoints || 0) + correctAnswers * 50,
                };

                return {
                    ...prev,
                    points: currentPoints + correctAnswers * 50,
                    streak: scorePercentage === 100 ? currentStreak + 1 : currentStreak,
                    progress: {
            ...
            if (!triggeredAchievements.has(lastMsg.id)) {
                ...
                setStudent(prev => ({
                    ...
                    gamification: {
                      ...(prev.gamification || { streak: 0, lastActiveDate: '', badges: [], totalPoints: 0, level: 1 }),
                      totalPoints: (prev.gamification?.totalPoints || 0) + 10
                    }
                }));
            }
        }
  ...
  // ==========================================================================
  // 🆕 HANDLER: Activar WebLLM Local (REAL)
  // ==========================================================================
  const handleActivateGemmaLocal = async () => {
    ...
    if (success) {
      ...
      // 🆕 Exponer flag global para que geminiService lo detecte
      (window as any).__forceLocalAI = true;
    } else {
      alert("No se pudo cargar el modelo local. Verifica que tu navegador soporte WebGPU.");
    }
  ...
  // ==========================================================================
  // HANDLER: Send Message (Online → Gemini | Offline → WebLLM)
  // ==========================================================================
  ...
  const handleEnterSubject = async (subject: string) => {
    if (subject === "Simulacro General ICFES") {
      ...
      setCurrentView('CLASSROOM');
      setIsSidebarOpen(false);
      setSimulationState({ isActive: true, currentQuestion: 1, totalQuestions: 5 });
  ...
  const handleStartModuleTool = async (module: SubjectModule, tool: NotebookTool) => {
    ...
    const materials = []; // TODO: Fix offline materials loading
  ...
  if (isInitializing) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-50 dark:bg-slate-900">
        ...
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Cargando EduGlobal365...</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Preparando tu espacio de aprendizaje offline</p>
      </div>
    );
  }
  ...
  if (initError) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-50 dark:bg-slate-900 p-4">
        ...
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 text-center">{initError}</h2>
        <button 
      ...
      <RewardShop 
        ...
        onPurchase={(cost, itemName) => {
          setStudent(prev => ({ 
            ...
            gamification: {
              ...(prev.gamification || { streak: 0, lastActiveDate: '', badges: [], totalPoints: 0, level: 1 }),
              totalPoints: (prev.gamification?.totalPoints || 0) - cost
            }
          }));
          setCurrentAchievement({
             id: generateUUID(),
    ...
    <div className="flex flex-col h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 overflow-hidden">
      ...
      <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-3 flex items-center justify-between shadow-sm z-10 sticky top-0 transition-colors duration-300">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* 🆕 Gemma Local WebGPU Engine - AHORA REAL */}
          <button
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        \`}>
          <div className="p-4 border-b border-slate-100 dark:border-slate-700">
             ...
             {/* 🆕 ZPD Badge */}
             {student.zpd && (
               <div className="mt-2 inline-flex items-center gap-1 px-2 py-0.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 text-[10px] font-bold rounded-full border border-indigo-100 dark:border-indigo-800">
                 <span>🧠</span> Nivel ZPD: {student.zpd.level}
               </div>
             )}
